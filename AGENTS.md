...
- All six payroll destinations are embedded in `admin.html` — the sub-nav (`payrollSubTabsHtml`) sits at the **bottom** of the payroll content area (`payrollSubTab` state): **01 General Payroll** → **02 Taxes & Incentives** → **03 Final Review** → **💸 Quick Pay** → **📋 Payroll History** → **🚪 Quit Pay**. Clicking a sub-tab re-renders only `.payroll-content` inside `#tabContent` — the admin header and main menu never unmount.
