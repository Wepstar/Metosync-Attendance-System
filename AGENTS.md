...
- All six payroll destinations are embedded in `admin.html` — the sub-nav (`payrollSubTabsHtml`) sits at the **bottom** of the payroll content area (`payrollSubTab` state): **01 General Payroll** → **02 Taxes & Incentives** → **03 Final Review** → **� Budgets** → **�💸 Quick Pay** → **📋 Payroll History** → **🚪 Quit Pay**. Clicking a sub-tab re-renders only `.payroll-content` inside `#tabContent` — the admin header and main menu never unmount.
- `payroll.html`, `taxes.html`, and `review.html` still exist but immediately `location.replace` to `admin.html?tab=payroll&sub=general|taxes|review` — they can never render their standalone workspace chrome.
